...
int find_father(std::vector<int> &f, int x) {
    return f[x] == x? x: (f[x] = find_father(f, f[x]));
}

void merge_father(std::vector<int> &f, int x, int y) {
    x = find_father(f, x);
    y = find_father(f, y);
    if (x != y) {
        f[x] = y;
...
at::Tensor connect_all_components(at::Tensor xyz, at::Tensor faces) {
    const int B = xyz.size(0);
    const int N = xyz.size(1);
    const int M = faces.size(1);

    std::vector<int> father(N);
    std::vector<std::vector<int> > scc(N);
    std::vector<int> added_faces;
    for (int i = 0; i < N; ++i) {
        father[i] = i;
    }
    for (int i = 0; i < M; ++i) {
        merge_father(father, faces[0][i][0].item<int>(), faces[0][i][1].item<int>());
        merge_father(father, faces[0][i][0].item<int>(), faces[0][i][2].item<int>());
        merge_father(father, faces[0][i][1].item<int>(), faces[0][i][2].item<int>());
    }
    for (int i = 0; i < N; ++i) {
        father[i] = find_father(father, i);
        scc[father[i]].push_back(i);
    }
    for (int i = 0; i < N; ++i) {
        if (scc[i].size() == 0) {
            continue;
...
//            DEBUG(i, j, scc[i].size(), scc[j].size());
            if (scc[j].size() > 1) {
                added_faces.push_back(scc[i][0]);
                added_faces.push_back(scc[j][0]);
                added_faces.push_back(scc[j][1]);
                success = true;
                break;
            } else if (scc[i].size() > 1) {
                added_faces.push_back(scc[i][0]);
                added_faces.push_back(scc[i][1]);
                added_faces.push_back(scc[j][0]);
                success = true;
                break;
            }
        ...
        if (!success) {
            DEBUG("hey", i, N, scc[i].size());
        }
    }
    DEBUG(added_faces[0], added_faces[1], added_faces[2]);
    at::Tensor faces_2 = torch::from_blob(added_faces.data(), {1, added_faces.size() / 3, 3}, at::device(at::kCPU).dtype(at::ScalarType::Int));
    DEBUG(faces_2.sizes(), faces_2[0][0]);
    faces = torch::concat({faces, faces_2}, 1);
...
void arap_deform_cpu(at::Tensor xyz, at::Tensor faces, at::Tensor handles, at::Tensor moves,
                     at::Tensor new_xyz) {
//    faces = connect_all_components(xyz, faces);

    const int B = xyz.size(0);
    const int N = xyz.size(1);
    const int M = faces.size(1);
    const int N_handles = handles.size(1);

    for (int b = 0; b < B; ++b) {
        Eigen::MatrixXd vertices(N, 3);
        for (int i = 0; i < N; ++i) {
            for (int j = 0; j < 3; ++j) {
                vertices(i, j) = xyz[b][i][j].item<float>();
            }
        }
        Eigen::MatrixXi tris(M, 3);
        for (int i = 0; i < M; ++i) {
            for (int j = 0; j < 3; ++j) {
                tris(i, j) = faces[b][i][j].item<int>();
            }
        }
        std::set<int> handles_vec(handles[b].data_ptr<int>(), handles[b].data_ptr<int>() + N_handles);
        Eigen::Vector3d move;
        for (int i = 0; i < 3; ++i) {
            move(i) = moves[b][i].item<int>();
        }
        std::set<int> fixedVertices;

        ARAP arap(vertices, tris);
        Eigen::MatrixXd new_vertices = arap.deform(fixedVertices, handles_vec, move);
        for (int i = 0; i < N; ++i) {
            for (int j = 0; j < 3; ++j) {
                new_xyz[b][i][j] = new_vertices(i, j);
            }
        }
    }
}
...
Eigen::MatrixXd ARAP::deform(std::set<int> &fixedPoints, std::set<int> &handles, Eigen::Vector3d &move)
{
    ...
    fillConstraints(fixedPoints, handles);    // constrains = fixedpoints | handles
    fillFreeVariables();                           // freeVariables = {0,1,2,...,V-1} / constrains
